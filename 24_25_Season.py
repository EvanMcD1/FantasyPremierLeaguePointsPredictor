...
class FPLPredictor:
    ...
    def load_xa_multipliers_data(self, gameweek):
        multipliers_file = f'expected_assists_multipliers_24_25_gw{gameweek - 1}.csv'
        if os.path.exists(multipliers_file):
            return pd.read_csv(multipliers_file)
        else:
            gameweek -= 1
    ...
    def load_xgc_multipliers_data(self, gameweek):
        multipliers_file = f'expected_goals_conceded_multipliers_24_25_gw{gameweek - 1}.csv'
        if os.path.exists(multipliers_file):
            return pd.read_csv(multipliers_file)
        else:
            gameweek -= 1
    ...
    def get_player_position(self, player_name, gameweek):
        file_path = '/Users/evanmcdermid/PycharmProjects/FantasyPremierLeague/Fantasy-Premier-League-master/data/2024-25/cleaned_players.csv'
        data = pd.read_csv(file_path)
        if {'first_name','second_name', 'element_type'}.issubset(data.columns):
            player_data = data[(data['first_name'] + ' ' + data['second_name']) == player_name]
            if not player_data.empty:
                return player_data['element_type'].values[0]
        return None
    ...
    def get_cleansheet_percent(self, player_name, gameweek):
        try:
            cs_data = pd.read_csv(f"gw{gameweek}cleansheet.csv")
    ...
    def minutes_per_start(self, player_name, consolidated_csv_path):
        ...
        if {'player_name', 'avg_minutes_per_game'}.issubset(data.columns):
            # Filter the data for the specific player by player_id
            player_data = data[data['player_name'] == player_name]

            if not player_data.empty:
                # Return the average minutes per game for the player
                return player_data['avg_minutes_per_game'].values[0]
            else:
                return 0
    ...
    def expected_points(self, firstname, secondname, gameweek, expectedminutes):
        ...
        data=pd.read_csv('/Users/evanmcdermid/PycharmProjects/FantasyPremierLeague/2023-24_player_points_per_minute_data.csv')
        ...
        goals_points = self.expected_goals_points(player_name, self.gw_files, expectedminutes,
                                                  None, is_home, player_position, gameweek)*xGmultiplier
        assist_points = self.expected_assists_points(player_name, self.gw_files, expectedminutes,
                                                    None, is_home, player_position, gameweek)*xGmultiplier
        cleansheet_points = self.expected_cleansheet_points(player_name, self.gw_files,
                                                            expectedminutes, player_position, gameweek)
        bonus_points = self.expected_bonus_points(player_name, self.gw_files, expectedminutes)
        save_points = self.expected_save_points(player_name, self.gw_files, expectedminutes)
        yellowcard_points = self.yellow_card_points(player_name, self.gw_files, expectedminutes,
                                                    player_position)
    ...
    def combine_gw_data(self, gameweek):
        ...
        file_output_path = (f'24_25_combined_gw{gameweek}_data.csv')
